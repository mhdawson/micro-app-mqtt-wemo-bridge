...
Server.startServer = function(server) {
  ...
  mqttClient.on('message', (topic, message) => {
    if (deviceMap[topic]) {
      ...
      wemoClient.discover((error, deviceInfo) => {
        if (error) {
          pushActivity('wemo client setup failed:' +  error);
          return;
        }

        if (deviceInfo.friendlyName === device.friendlyName) {
          try {  
            const wemoDevice = wemoClient.client(deviceInfo);
            if (message == 'on') {
            ...
            pushActivity('wemo request failed:' +  error);
          };
        }
      });
    } else {
